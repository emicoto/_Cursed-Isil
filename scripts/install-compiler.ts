...
utils.logInfo(
	"Copyright(c) 2014 - 2018 Thomas Michael Edwards <thomasmedwards@gmail.com>.\nAll rights reserved."
);

downloadAndExtract(tweegoLink, utils.tweegoZip).then(() => {
	utils.logAction(
		"Downloading the story formats from",
		utils.links.storyFormats
	);
	...
	downloadAndExtract(
		...
		if (process.platform === "win32") {
			utils.logWarning(
				"If you receive an error about SugarCube not being available, rename the folder " +
					'"sugarcube-2" to "SugarCube" and try again.'
			);
		}
	});
});
			...
			yauzl.open(filePath, { lazyEntries: true }, (error, zip) => {
				...
				if (zip) {
					...
					zip.on("entry", (entry: any) =>
						isFolder(entry) ? zip.readEntry() : unzipEntry(zip, entry)
					);
					zip.on("end", resolve);
					zip.readEntry();
				}
		...
		zip.openReadStream(
			...
			(
				...
				const fileFolder = path.resolve(
					folder,
					getFileDirectory(entry.fileName)
				);
