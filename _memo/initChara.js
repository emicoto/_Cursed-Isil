...
F.initChara.Ayres = function(){
    ...
    chara.flag['cursedLv'] = 0;
    chara.flag['cursedMP'] = 0;
...
F.initChara.Isil = function(){
    const chara = new Chara("Isil", "伊希露", "male", "elvin", "Isil")
	.initChara("neko")
	.setNames({ m: "梅尔", s: "艾西利特" })
	.setBirth([3998, 6, 17])
	.setTitle("学者")
	.setJob("scholar")
	.setTraits(['纤弱','不耐疼','M体质','A名器','易伤体质','淡漠','冷静','理智','不过线','性保守','高傲','低调','高自尊','坚韧','贞洁'])
...
F.initChara.Besta = function(){
    ...
    F.setScar(chara, 'face', 'scar', 'never');
    F.setScar(chara, 'back', 'scar', 'never');
    F.setScar(chara, 'back', 'scar', 'never');
    F.setScar(chara, 'waistback', 'scar', 'never');
    F.setScar(chara, 'abdomen', 'scar', 'never');
    F.setScar(chara, 'breast', 'scar', 'never');
    F.setScar(chara, 'arms', 'scar', 'never');
    F.countSkin(chara, 0);
...
F.initChara.Nanaly = function(){
	const chara = new Chara('Nanaly', '娜娜莉', 'female', 'dracon')
		...
		chara.resetVirginity = function(){
			if(this.gender !== 'male')
				this.setVirginity(['anal', '不明' ,'以某人为幻想对象，用震动棒自行破处'])
		}
