...
F.Evole = function(type){
	...
	const p = {
		len:[1, '变长'], thick:[2, '变粗'], num:[8, '变多'], form:[5, '突变'] , drug:[3], hypnos:[16], cum:[1], scale:[500, '进化']
	}
	...
	if(m.MP >= mp && m.abl[type] < max ){
		link = `　<<link '${p[type][1] ? p[type][1] : '升级'}  ${mp}P' $passage>><<set $cursedLord.abl['${type}'] += 1>><<set $cursedLord.MP -= ${mp}>><</link>>`
	}
	else if(m.abl >= max ){
